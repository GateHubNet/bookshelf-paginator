...
function Paginator(model, options) {
  ...
  if (typeof this.Bookshelf.model(this.model) === 'undefined') {
...
Paginator.prototype.init = function(options) {
  ...
  this.modelInstance = this.Bookshelf.model(this.model).forge();
...
Paginator.prototype.prepateQuery = function() {
  ...
  baseQuery = this.Bookshelf.knex(this.modelInstance.tableName);
  ...
  this.countQuery = baseQuery.clone()
    .select(this.Bookshelf.knex.raw('COUNT(DISTINCT _result.id) as count'))
